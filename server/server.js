...
const serverRenderer = (req, res, next) => {
  fs.readFile(path.resolve("./build/index.html"), "utf8", (err, data) => {
    ...
    return res.send(
      `<div id="root" data-props='{ "product": "${
        req.query.product
      }" }'>${ReactDOMServer.renderToString(<App product={req.query.product} />)}
      ...
      <script src="/static/js/2.3f64a24d.chunk.js"></script>
      <script src="/static/js/main.d3f79bd0.chunk.js"></script>
      </div>`
      // data.replace(
      //   '<div id="root"></div>',
      //   `<div id="root" data-props='{ "product": "${req.query.product}" }'>${ReactDOMServer.renderToString(
      //     <App />
      //   )}</div>`
      // )
    );
